...
class AppStore extends Component {
  state = {searchInput: '', appCategory: appsList[0].category}
  ...
  getAppsList = searchApps => {
    const {appCategory} = this.state
    const filter = searchApps.filter(
  ...
  render() {
    const {searchInput, appCategory} = this.state
    const searchResult = this.getSearchResult()
    const filteredAppsList = this.getAppsList(searchResult)
        ...
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              key={eachTab.tabId}
              isActive={appCategory === eachTab.tabId}
            />
          ))}
        </ul>
